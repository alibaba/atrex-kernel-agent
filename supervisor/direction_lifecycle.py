...
def advance_direction(
    statuses: dict[str, str], started: set[str], direction_id: str, action: str,
) -> None:
    ...
    if not isinstance(action, str) or action not in DIRECTION_STATUS:
        raise ValueError("Direction action must be propose, start, complete, abandon, block, or defer")
    ...
    if action == "propose":
        ...
        active = [key for key, value in statuses.items() if value == "in_progress" and key != direction_id]
        if active:
            raise AgentRequestError(
                "only one Direction may be in progress; close or defer "
                f"{active[0]} before starting another",
                code="direction_in_progress", direction_id=active[0],
                next_action=(
        ...
        if direction_id not in started and len(started) >= 3:
            raise AgentRequestError(
                ...
                code="direction_limit_exceeded", started_direction_ids=sorted(started),
                next_action=(
