        ...
        for root in self.roots:
            ...
            try:
                if not path.is_file() or path.stat().st_size > MAX_CHECKPOINT_BYTES:
                    raise ValueError("invalid checkpoint size or type")
                ...
                from long_horizon.verifier import _payload_from_stdout

                if _payload_from_stdout(value["stdout"]) != value["payload"]:
