...
"""flash_attn_func no-mask forward - TUNED FOR AMD MI308X (CDNA3 / gfx942).

HARDWARE-SPECIFIC + NO-MASK VARIANT. This file is archived as a separate
MI308X no-mask reference implementation and intentionally coexists with:

- `reference-kernels/amd/cdna3/flydsl/FlyDSL/flash_attn_func_mi308x.py`
  (causal + GQA tuned variant)
- `reference-kernels/amd/cdna3/flydsl/FlyDSL/flash_attn_func_mask_mi308x.py`
  (bit-packed arbitrary mask variant)
...
- Inputs are bf16 in the tuned benchmark; MFMA accumulates in fp32, online
  softmax is fp32, and output is converted back to bf16.
- Target shape for this archive: B=1024, H=8, S=316 padded to S_pad=320,
  D=64, BLOCK_M=128, BLOCK_N=64.

Key MI308X no-mask tunings:
- `head_dim=64` native path: `D_CHUNKS=2` with `mfma_f32_32x32x8_bf16`.
- K/V use separate LDS regions; no D=64->128 padding waste.
- `K_PAD=2`, `VT_STRIDE=BLOCK_N+2`, LDS footprint 16896 B.
- `waves_per_eu=1`; verified final resources: VGPR=128, AccVGPR=8,
  SGPR=112, scratch=0.
- QK prefetch depth remains 2; CK-style depth 3 regressed in this pipeline.
- `FLYDSL_FLASH_ATTN_FUNC_REDUCE_MODE=ds_bpermute_lgkm_sum` is the final
  default. Rowmax keeps the normal `rocdl.ds_bpermute`; rowsum uses inline
  `ds_bpermute_b32` plus `s_waitcnt lgkmcnt(0)`.
- The ISA schedule deliberately delays `vmcnt(0)` until after independent
  `v_pk_mul_f32` rescale work, just before V data is consumed by
  `ds_swizzle_b32`.
- `NOMASK_SOFTMAX_BARRIER` remains enabled with mask 0; disabling it or using
  CK-like barrier masks regressed.

Final verified result on MI308X / gfx942:
- Correctness: rel_err vs fp32 reference = 0.017906, rel_err vs bf16
  reference = 0.018016, PASS under the 0.02 threshold.
- `rocprofv3 --kernel-trace`: P50 3.479213 ms, 60.189822 TFLOPS.
- Authoritative ATT capture:
  `profiles/nomask_v6_final_lgkm_sum_default/att/rocprof_att_codeobj4_kernel_body.s`
  with MD5 `333eb8ff20e3a0911833b75a2cf86ded`.
- Dtype note: the older wiki CK/SDPA no-mask number (`2.94 ms / 71.3 TFLOPS`)
  was FP16, not BF16. It is not used as the acceptance target for this BF16
  archive; the BF16 no-mask implementation is recorded as meeting the current
  BF16 objective. Capture an apples-to-apples BF16 CK baseline before making a
  CK95 comparison.

Algorithm:
- True MFMA32 remap: `mfma_f32_32x32x8bf16` for both GEMM stages.
- Tile shape: BLOCK_M=128, BLOCK_N=64, 4 waves (256 threads).
...
- P is kept in registers and fed directly to GEMM2 (`V^T @ P`) without LDS
  roundtrip.
- K and V use separate LDS regions with global-to-LDS prefetch and XOR swizzle.
...
def build_flash_attn_func_module_primary(
    ...
    ENABLE_NOMASK_SOFTMAX_BARRIER = (
        os.getenv("FLYDSL_FLASH_ATTN_FUNC_NOMASK_SOFTMAX_BARRIER", "1") == "1"
    )
    ...
    QK_PREFETCH_DEPTH = int(os.getenv("FLYDSL_FLASH_ATTN_FUNC_QK_PREFETCH_DEPTH", "2"))
    ...
    # No-mask D64 tuned default: K_PAD=2 gives a 66-element K row stride
    # (132 B = 33 dwords), which is coprime with the 32-bank LDS layout and
    # was the best verified setting for this archived shape.
    _kpad_default = "2" if (not USE_HW_TR) else "0"
    ...
    if USE_HW_TR:
        ...
        # V11a (2026-05-10) re-tested +8 on the adjacent bit-packed mask shape:
        # bank conflicts were ~5x worse and wall clock regressed. This no-mask
        # archive keeps the same VT_STRIDE=66 rule.
        VT_STRIDE = BLOCK_N + 2
        V_STRIDE = VT_STRIDE
    ...
    def flash_attn_func_kernel(
        ...
        # ---- KV loop upper bound ----
        # No-mask non-causal path: iterate over the full sequence.
        kv_upper = seq_len_v
        ...
        for kv_block_start, inner_iter_args, loop_results in scf.for_(
            ...
            for kv_sub in range_constexpr(N_SUBTILES):
                ...
                # ==== Optional bit-packed mask path (compile-time disabled here) ====
                if const_expr(HAS_MASK):
                    rocdl.sched_barrier(0)
                if const_expr(HAS_MASK):
