...
Target: MI308X (gfx942), DK=DV=128, BT=64, block_DV=32 small-H fast path
"""
import torch
import triton
...
def build_megakernel(
    ...
    allocator = SmemAllocator(None, arch=gpu_arch, global_sym_name="megakernel_mi308x_v2_bdv32_smem")
        ...
        def producer_load_q_a_gb(chunk_base_val):
            ...
            if PRODUCER_STAGE_Q:
                for ei in range_constexpr(2):
                    ...
                    q_vec = buffer_ops.buffer_load(rsrc_q, q_off, vec_width=8, dtype=T.bf16)
                    vector.store(q_vec, lds_q, [to_idx(q_row * STRIDE_Q + p_col0)])
                    q_vec2 = buffer_ops.buffer_load(rsrc_q, q_off + 8, vec_width=8, dtype=T.bf16)
        ...
        def producer_load_kt(chunk_base_val):
            ...
            if PRODUCER_STAGE_KT:
                for ei in range_constexpr(2):
                    ...
                    k_vec2 = buffer_ops.buffer_load(rsrc_k, k_off + 8, vec_width=8, dtype=T.bf16)
            ...
            with ir.InsertionPoint(role_if.then_block):
                ...
                for nt in range_constexpr(NT_BDV):
                    for ii in range_constexpr(4):
                        ...
                        v_preloads[nt][ii] = buffer_ops.buffer_load(rsrc_v, v_off, vec_width=1, dtype=T.bf16)
                ...
                for kt in range_constexpr(8):  # K/16 = 8 reduction tiles on gfx942
                    ...
                    a_K = buffer_ops.buffer_load(rsrc_k, k_gmem_off, vec_width=4, dtype=T.bf16)
...
def megakernel_fwd(
    ...
    fn(
        ...
        ssm_state_stride, 0, total_t_for_store, scale, T_total, N,
    )
