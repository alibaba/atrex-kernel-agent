#!/usr/bin/env python3
"""FlyDSL megakernel V2a for MI308X (CDNA3, gfx942).
...
Starting from the MI308 V1 baseline:
  ...
  - LDS: 88KB → 62.8KB (fits MI308X 64KB/CU)
...
Target: MI308X (gfx942), DK=DV=128, BT=64, block_DV=64, LDS ≈ 62.8KB
"""
import importlib
import torch
import triton
...
def build_megakernel(
    ...
    allocator = SmemAllocator(None, arch=gpu_arch, global_sym_name="megakernel_mi308x_v2_smem")
    ...
    def megakernel_fn(
        ...
        def mfma(a, b, c):
            r = rocdl.mfma_f32_16x16x16bf16_1k(
                v4f32, [vector.bitcast(v4i16, a), vector.bitcast(v4i16, b), c, _z, _z, _z]
            )
        ...
        def load_b_k16_scalar(lds_ref, stride, k_base, n_col):
            ...
            for ii in range_constexpr(4):
                val_vec = vector.load_op(v1bf16, lds_ref, [to_idx((k_base + ii) * stride + n_col)])
                vals.append(vector.extract(val_vec, static_position=[0], dynamic_position=[]))
        ...
        def load_a_bt_scalar(lds_ref, stride, bt_base, dk_col):
            vals = []
            for ii in range_constexpr(4):
                val_vec = vector.load_op(v1bf16, lds_ref, [to_idx((bt_base + ii) * stride + dk_col)])
                vals.append(vector.extract(val_vec, static_position=[0], dynamic_position=[]))
            return vector.from_elements(v4bf16, vals)
        ...
        if is_varlen:
            ...
            rsrc_co = buffer_ops.create_buffer_resource(chunk_offsets_ptr, max_size=True)
            bos = arith.trunci(T.i32, buffer_ops.buffer_load(rsrc_cu, i_b, vec_width=1, dtype=T.i64))
            eos = arith.trunci(T.i32, buffer_ops.buffer_load(rsrc_cu, i_b + 1, vec_width=1, dtype=T.i64))
            T_actual = eos - bos
            NT = (T_actual + BT - 1) // BT
        ...
        rsrc_prefix = buffer_ops.create_buffer_resource(prefix_lengths_ptr, max_size=True)
        ...
        lds_beta = SmemPtr(lds_base, lds_beta_off, T.bf16, shape=(LDS_BETA_ELEMS,)).get()
        ...
        def maybe_store_h_to_ssm(h_vals, chunk_idx_i32):
            ...
            prefix = buffer_ops.buffer_load(
                rsrc_prefix, i_b, vec_width=1, dtype=T.i32)
        ...
        is_compute = arith.cmpi(arith.CmpIPredicate.slt, wave_id, arith.constant(4, type=T.i32))
        prod_wave = wave_id - 4
                ...
                for mc in range_constexpr(2):
                    for nt in range_constexpr(NT_BDV):
                        ...
                        for ii in range_constexpr(4):
                            ...
                            h0_vals.append(buffer_ops.buffer_load(rsrc_h0, h0_off, vec_width=1, dtype=T.f32))
        ...
        def producer_load_kt(chunk_base_val):
            ...
            if PRODUCER_STAGE_KT:
                for ei in range_constexpr(2):
                    ...
                    vector.store(k_vec2, lds_kT, [to_idx(k_row * STRIDE_KT + p_col0 + 8)])
        ...
        def producer_load_kt_regs(chunk_base_val):
            ...
            if PRODUCER_STAGE_KT:
                for ei in range_constexpr(2):
                    ...
                    kt_vecs2[ei] = buffer_load_bf16_v8_or_zero(rsrc_k, k_off + 8, k_valid)
        ...
        def producer_store_kt_regs(kt_vecs, kt_vecs2):
            if PRODUCER_STAGE_KT:
                for ei in range_constexpr(2):
                    ...
                    vector.store(
                        kt_vecs[ei], lds_kT,
                        [to_idx(k_row * STRIDE_KT + p_col0)])
                    vector.store(
                        kt_vecs2[ei], lds_kT,
                        [to_idx(k_row * STRIDE_KT + p_col0 + 8)])
        ...
        def compute_load_q_kt_a(chunk_base_val):
            ...
            if COMPUTE_STAGE_KT:
                for ei in range_constexpr(2):
                    ...
                    vector.store(k_vec2, lds_kT, [to_idx(k_row * STRIDE_KT + q_col0 + 8)])
            ...
            with ir.InsertionPoint(role_if.then_block):
                ...
                for nt in range_constexpr(NT_BDV):
                    for ii in range_constexpr(4):
                        ...
                        v_preloads[nt][ii] = buffer_load_bf16_or_zero(rsrc_v, v_off, v_valid)

                last_idx_candidate = arith.ArithValue(arith.muli((i_c_i32 + 1), arith.constant(BT, type=T.i32)))
                T_act_v = arith.ArithValue(T_actual)
                cmp_lt = arith.cmpi(arith.CmpIPredicate.slt, last_idx_candidate, T_act_v)
                ...
                for ii in range_constexpr(4):
                    ...
                    g_row_vals[ii] = vector.extract(g_vec, static_position=[0], dynamic_position=[])

                g_col_vals = [None] * 4
                for nt in range_constexpr(4):
                    g_abs_col = nt * 16 + lane % 16
                    g_vec = vector.load_op(v1f32, lds_g, [to_idx(g_abs_col)])
                    g_col_vals[nt] = vector.extract(g_vec, static_position=[0], dynamic_position=[])

                beta_col_vals = [None] * 4
                for nt in range_constexpr(4):
                    beta_abs_col = nt * 16 + lane % 16
                    beta_vec = vector.load_op(v1bf16, lds_beta, [to_idx(beta_abs_col)])
                    beta_col_vals[nt] = vector.extract(beta_vec, static_position=[0], dynamic_position=[])

                # ════════════════════════════════════════
                # PHASE B: GEMM1 U=K@h, GEMM2 O_inter=Q@h, W, O scaling
                # GEMM1: K loaded from GMEM (no lds_k_row on MI308X)
                # ════════════════════════════════════════
                ...
                for kt in range_constexpr(8):  # K/16 = 8 reduction tiles on gfx942
                    ...
                    k_gmem_off = (chunk_base + k_row_idx) * stride_k + k_head * K + k_col_idx
                    a_K = buffer_load_bf16_v4_or_zero(rsrc_k, k_gmem_off, k_row_valid)

                    a_q_off = (wave_id * 16 + lane % 16) * STRIDE_Q + kt * 16 + (lane // 16) * 4
                ...
                g_exp_row = [math_dialect.exp2(g_row_vals[ii]) for ii in range_constexpr(4)]
                ...
                for nt in range_constexpr(NT_BDV):
                    for ii in range_constexpr(4):
                        ...
                        vector.store(
                            ...
                            lds_w, [to_idx(w_col * STRIDE_H + w_row)])
                ...
                for nt in range_constexpr(4):
                    for ii in range_constexpr(4):
                        ...
                        g_gate = math_dialect.exp2(arith.subf(g_row_vals[ii], g_col_vals[nt]))
                        ...
                        a_elem = arith.extf(T.f32, vector.extract(a_elem_vec, static_position=[0], dynamic_position=[]))
                ...
                for kt in range_constexpr(4):  # BT/16 = 4 reduction tiles on gfx942
                    ...
                    a_ag_off = (wave_id * 16 + lane % 16) * STRIDE_A + kt * 16 + (lane // 16) * 4
                ...
                for kt in range_constexpr(4):
                    ...
                    a_pg_off = (wave_id * 16 + lane % 16) * STRIDE_A + kt * 16 + (lane // 16) * 4
                ...
                for mc in range_constexpr(2):
                    for kt in range_constexpr(4):  # BT/16 = 4 reduction tiles on gfx942
                        ...
                        for nt in range_constexpr(NT_BDV):
                            h_acc[mc * NT_BDV + nt] = mfma(a_kT, b_vn[nt], h_acc[mc * NT_BDV + nt])
    ...
    def launch(
        ...
        a_ptr: fx.Tensor, g_ptr: fx.Tensor, beta_ptr: fx.Tensor,
        o_ptr: fx.Tensor, h0_ptr: fx.Tensor, ht_ptr: fx.Tensor,
        cu_seqlens_ptr: fx.Tensor, chunk_offsets_ptr: fx.Tensor,
        prefix_lengths_ptr: fx.Tensor, block_map_ptr: fx.Tensor,
        ssm_states_ptr: fx.Tensor, max_block_size: fx.Int32,
        seq_size_per_block: fx.Int32, ssm_state_stride: fx.Int32,
        token_offset: fx.Int32, total_T_for_store: fx.Int32,
        scale: fx.Float32, T_val: fx.Int32, N_val: fx.Int32,
        ...
        megakernel_fn(
            q_ptr, k_ptr, v_ptr, a_ptr, g_ptr, beta_ptr, o_ptr,
            h0_ptr, ht_ptr, cu_seqlens_ptr, chunk_offsets_ptr,
            prefix_lengths_ptr, block_map_ptr, ssm_states_ptr,
            max_block_size, seq_size_per_block, ssm_state_stride,
            token_offset, total_T_for_store, scale, T_val,
        ).launch(
            grid=(grid_x, 1, 1),
...
def _launch_tail_safe_into(
    q, k, v, a, g, beta, scale, initial_state, output_final_state,
    cu_seqlens, chunk_offsets, o,
    prefix_lengths=None, block_map=None, ssm_states=None,
    seq_size_per_block=None, token_offset=0, total_t_for_store=0,
):
    ...
    if cu_seqlens is not None:
        ...
        if chunk_offsets is None:
            chunk_offsets = torch.cat([cu_seqlens.new_tensor([0]),
                                       triton.cdiv(lens, BT)]).cumsum(-1)
    else:
        N = B
    cache_key = (H, Hg, K, V, use_h0, use_vl, store_ssm_state, ssm_states.dtype if store_ssm_state else None)
    ...
    ht = torch.empty(N, H, K, V, device=q.device, dtype=torch.float32) if output_final_state else None
    ...
    fn(
        q, k, v, a, g, beta, o, h0_arg, ht_arg, cu_arg, co_arg,
        prefix_arg, block_map_arg, ssm_arg, max_block_size, seq_block,
        ssm_state_stride, token_offset, total_t_for_store, scale, T_total, N,
    )
...
def _launch_fast_into(
    ...
    fast = importlib.import_module(
        "fused_fwd_mi308x_v2_bdv32_fast"
        if _use_bdv32_fast_path(H, Hg, K, V)
        else "fused_fwd_mi308x_v2_fast"
    )
...
def _tail_split_single_sequence(
    q, k, v, a, g, beta, scale, initial_state, output_final_state, prefix_len,
    prefix_lengths=None, block_map=None, ssm_states=None, seq_size_per_block=None,
):
    ...
    h_prefix = _launch_fast_into(
        q[:, :prefix_len], k[:, :prefix_len], v[:, :prefix_len],
        a[:, :prefix_len], g[:, :prefix_len], beta[:, :prefix_len],
        scale, initial_state, True, None, None, o[:, :prefix_len],
    ...
    ht = _launch_tail_safe_into(
        q[:, prefix_len:], k[:, prefix_len:], v[:, prefix_len:],
        a[:, prefix_len:], g[:, prefix_len:], beta[:, prefix_len:],
        scale, h_prefix, output_final_state, None, None, o[:, prefix_len:],
        prefix_lengths=prefix_lengths,
        block_map=block_map,
        ssm_states=ssm_states,
        seq_size_per_block=seq_size_per_block,
...
def megakernel_fwd(
    ...
    """Fused recompute_wu + fwd_h + fwd_o megakernel (MI308X V1)."""
    B, T_total, _, K = q.shape
    _, _, H, V = v.shape
    ...
    if cu_seqlens is None:
        if T_total % BT == 0:
            ...
            ht = _launch_fast_into(
                q, k, v, a, g, beta, scale, initial_state,
                output_final_state, None, None, o,
                prefix_lengths=prefix_lengths,
                block_map=block_map,
                ssm_states=ssm_states,
                seq_size_per_block=seq_size_per_block,
    ...
    if len(cu_seqlens) == 2:
        ...
        if T_total % BT == 0:
            ...
            ht = _launch_fast_into(
                q, k, v, a, g, beta, scale, initial_state,
                output_final_state, cu_seqlens, chunk_offsets, o,
                prefix_lengths=prefix_lengths,
                block_map=block_map,
                ssm_states=ssm_states,
                seq_size_per_block=seq_size_per_block,
