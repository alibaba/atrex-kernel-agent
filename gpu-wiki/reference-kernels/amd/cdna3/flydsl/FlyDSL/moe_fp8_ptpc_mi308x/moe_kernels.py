...
"""FlyDSL FP8 PTPC MoE kernel management for MI308X.
...
def get_flydsl_stage1_kernels_bf16(out_dtype: str) -> Dict[str, Dict]:
    """Return {kernelName: params} for dense bf16/bf16 stage1 configs."""
    kernels = {}
    a_dtype = "bf16"
    b_dtype = "bf16"
    tile_ms = [16, 32, 64, 128]
    tile_ns = [32, 64, 128, 256]
    tile_ks = [64, 128, 256]
    b_nts = [0, 2]
    xcd_swizzles = [0, 4]
...
def get_flydsl_stage1_kernels_fp8_ptpc(out_dtype: str) -> Dict[str, Dict]:
    """Return {kernelName: params} for FP8 PTPC stage1 configs."""
    kernels = {}
    a_dtype = "fp8"
    b_dtype = "fp8"
    tile_ms = [16, 32, 64, 128]
...
def get_flydsl_stage2_kernels_bf16(out_dtype: str) -> Dict[str, Dict]:
    """Return {kernelName: params} for dense bf16/bf16 stage2 configs."""
    kernels = {}
    a_dtype = "bf16"
    b_dtype = "bf16"
    tile_ms = [16, 32, 64, 128]
...
def get_flydsl_stage2_kernels_fp8_ptpc(out_dtype: str) -> Dict[str, Dict]:
    """Return {kernelName: params} for FP8 PTPC stage2 configs."""
    kernels = {}
    a_dtype = "fp8"
    b_dtype = "fp8"
    tile_ms = [16, 32, 64, 128]
...
def _register_all_configs():
    ...
    for out in ("bf16", "f16"):
        ...
        _KERNEL_PARAMS.update(get_flydsl_stage1_kernels_fp8_ptpc(out))
        _KERNEL_PARAMS.update(get_flydsl_stage2_kernels_fp8_ptpc(out))
...
def compile_flydsl_moe_stage1(
    ...
    use_cshuffle_epilog: Optional[bool] = False,
...
def flydsl_moe_stage1(
    ...
    *,
    ...
    xcd_swizzle: int = 0,
    swiglu_limit: float = 0.0,
    grid_y_override: Optional[int] = None,
    use_cshuffle_epilog: Optional[bool] = False,
    assume_valid_grid: bool = False,
):
    ...
    if grid_y_override is None:
        _grid_y = min(_dense_blks, _all_blks)
    else:
        _grid_y = max(1, min(int(grid_y_override), _all_blks))
...
def flydsl_moe_stage2(
    ...
    *,
    ...
    elif accumulate and zero_output:
        out.fill_(0)
    ...
    if grid_y_override is not None:
        m_blocks = max(1, min(int(grid_y_override), sorted_expert_ids.shape[0]))
    if persist is True:
        _persist_m = -1
