---
name: hsaco-offline-launcher
description: In remote AMD GPU environments without Triton/Gluon, load precompiled .hsaco files and launch kernels. Supports two modes: C++ launcher mode and pure Python single-file embedding mode. Documents special handling for Gluon kernels and common precision pitfalls.
---

# HSACO Offline Launcher Skill
...
```bash
# Remote compilation
cd /tmp/tools && ROCM_PATH=/opt/rocm python3 setup.py build_ext --inplace
```
...
**Fix**: Retrieve the correct shared memory size from `compiled.metadata.shared`, do not rely on the `group_segment_fixed_size` in hsaco metadata (which is 0 when dynamic)### 4. Compiler Implicit Parameter Omission
