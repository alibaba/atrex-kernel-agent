---
name: hopper-triton-to-gluon-converter
description: Convert Triton kernels to Gluon kernels (NVIDIA Hopper sm_90 specific), with static knowledge and real-time learning capabilities.
---

# Triton → Gluon Conversion Skill (NVIDIA Hopper)

> **This Skill is specific to NVIDIA Hopper (sm_90, H20/H100/H200) targets.**
> For AMD GPUs, use sibling Skills: `cdna3-triton-to-gluon-converter.skill` (CDNA3/MI300) or `cdna4-triton-to-gluon-converter.skill` (CDNA4/MI355X)
>
> Common utilities and reference documentation are in the parent Skill `triton-to-gluon-converter.skill`.
...
See detailed patterns at: `references/patterns/pipeline.md`
...
See detailed patterns at: `references/patterns/matrix_multiply.md`
...
All layouts must be extracted from TTGIR via `tools/extract_ttgir.py`. See `references/layouts.md` for details.
...
| `tl.load` (pipeline data) | `async_copy.async_copy_global_to_shared` | CP_ASYNC DMA |For general API mapping, see the parent Skill at `references/porting_rules.md`.
For the complete Hopper mapping table, see: `references/api_mapping.md`

### 3. Conversion Modes

- **Memory Access Mode**: `references/patterns/memory_access.md`
- **Matrix Multiply Mode**: `references/patterns/matrix_multiply.md`
- **Pipeline Mode (num_stages>1)**: `references/patterns/pipeline.md`
- **Layout Reference**: `references/layouts.md`
...
4. ⚠️ Check the value of num_stages in the original Triton code:
   ...
   - num_stages > 1 → Set num_stages=1 in wrapper, but **must** manually implement pipeline in Gluon kernel
     using async_copy, see references/patterns/pipeline.md
   - ⚠️ Even with pipeline, if using gl.load+smem.store instead of async_copy, performance will still be 50%+ worse
5. Check api_mapping.md / parent porting_rules.md → Has mapping → Convert directly
6. No mapping → Call lookup_api.sh to query
7. After conversion, execute four verifications in sequence (see parent Skill)
```
...
|------|------|------|
| `references/api_mapping.md` | This Skill | Hopper API Quick Reference Table |
| `references/layouts.md` | This Skill | Hopper Layout Types and TTGIR Mapping |
| `references/patterns/` | This Skill | Hopper Conversion Modes (memory access, wgmma, async_copy pipelines) |
| `references/common_pitfalls.md` | This Skill | Hopper Common Errors and Solutions |
| `../references/porting_rules.md` | Parent Skill | General API Reference (includes Hopper annotations) |
| `../references/learning_guide.md` | Parent Skill | Unknown API Learning Process |
| `../references/verification_guide.md` | Parent Skill | Verification Methods and Failure Handling |
