...
| 4 | **Multi-shape correctness sweep**: validate V0 PASS on the full Path-1 shape matrix beyond the (M=6144, K=4096) headline shape | Deferred | validation work |
| 5 | **V_final perf matrix vs baseline**: cuda.Event + ncu DRAM throughput per shape, vs SDPA-only + memcpy-ceiling reference | Deferred | performance work |
| 6 | Final memory.md report appending Stage 4 results | Deferred | reporting work |
| 7 | Route 6 staged drafts plus the V0 reference kernel into the wiki archive | Deferred | wiki maintenance |
| 8 | Session-end summary: V_final, multi-shape pass rate, perf matrix, and wiki commit hash range | Deferred | reporting work |
