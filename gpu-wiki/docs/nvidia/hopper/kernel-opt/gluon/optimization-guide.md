...
> For AMD GPUs, use the [CDNA3](../../../../amd/cdna3/kernel-opt/gluon/) or
> [CDNA4](../../../../amd/cdna4/kernel-opt/gluon/) Gluon guide for the target architecture.
...
This guide assumes the input code is already a **correctly compilable and runnable Gluon kernel (Hopper target)**. If conversion from Triton to Gluon is needed, first use the Hopper conversion guidance in `docs/nvidia/hopper/conversion-guide.md`.

This guide can directly reference the following local wiki content to avoid duplication:
- **Hopper API Reference**: `docs/nvidia/hopper/api_mapping.md`
- **Layout Types**: `docs/nvidia/hopper/layouts.md`
- **Memory Access Patterns**: `docs/nvidia/hopper/memory_access.md`
- **Matrix Multiplication Patterns**: `docs/nvidia/hopper/matrix_multiply.md`
- **Pipeline Patterns**: `docs/nvidia/hopper/pipeline.md`
...
Read kernel code
  ...
  │            (docs/nvidia/hopper/ref-docs/gluon/linear_attention.md)
  │
  ├─ Has wgmma? Main loop iterates along K dimension without cross-iteration dependency?
  │   └─ YES → Standard GEMM (docs/nvidia/hopper/ref-docs/gluon/matmul.md)
  │
  ├─ No wgmma? Pure reduction / element-wise?
  │   └─ YES → Softmax/Reduce (softmax_reduce.md)
  │
  └─ None match
      └─ Fall back to common optimization (docs/nvidia/hopper/ref-docs/gluon/common_optimizations.md)
```
...
| Chunk Linear Attention / Recurrent | h[i+1]=f(h[i]); chunk_size split; iterative state matrix update | `docs/nvidia/hopper/ref-docs/gluon/linear_attention.md` |
| Standard GEMM | Contains wgmma; K-dimension iteration with no cross-iteration dependency | `docs/nvidia/hopper/ref-docs/gluon/matmul.md` |
...
> For detailed specifications, see `docs/nvidia/hopper/hardware-specs/hardware_specs_hopper.md`.
...
- **Latency-Bound and within 1.3× of theoretical lower bound** → Stop ISA-level optimization (see `docs/nvidia/hopper/ref-docs/gluon/linear_attention.md`)
- **Checklist exhausted** → Refer to the corresponding checklist by hit pattern

| Hit Pattern | Applicable Checklist |
|---------|---------------|
| Recurrent | Stopping conditions of `docs/nvidia/hopper/ref-docs/gluon/linear_attention.md` |
...
> For detailed profile interpretation guide, see `docs/nvidia/hopper/ref-docs/gluon/profiling_guide.md`.
...
1. **Pattern matched** → Follow the optimization strategy in the specialized documentation
   - Encountering generic ISA issues (coalesced access, load width, etc.) → Refer to `docs/nvidia/hopper/ref-docs/gluon/common_optimizations.md`
   - Encountering pitfalls → Refer to `docs/nvidia/hopper/ref-docs/gluon/pitfalls.md` or inline pitfall experiences in the specialization documentation

2. **No pattern matched** → Follow the optimization order in `docs/nvidia/hopper/ref-docs/gluon/common_optimizations.md`: 3.0 → 3.1 → ... → 3.6> See `docs/nvidia/hopper/ref-docs/gluon/common_optimizations.md` for the detailed optimization checklist.
...
| Document | Content |
|----------|---------|
| `docs/nvidia/hopper/hardware-specs/hardware_specs_hopper.md` | NVIDIA Hopper GPU hardware compute specification table (H20/H100/H200) |
| `docs/nvidia/hopper/ref-docs/gluon/profiling_guide.md` | Nsight Compute (ncu) profiling details |
| `docs/nvidia/hopper/ref-docs/gluon/isa_patterns.md` | Hopper sm_90 SASS instruction patterns and optimization reference |
| `docs/nvidia/hopper/ref-docs/gluon/common_optimizations.md` | General ISA optimization checklist (§3.0-3.6) + quick reference by bottleneck/type + Hopper vs AMD differences |
| `docs/nvidia/hopper/ref-docs/gluon/pitfalls.md` | Practical pitfall experience index (by pattern tag) |
| `docs/nvidia/hopper/ref-docs/gluon/matmul.md` | Standard GEMM optimization topic |
| `docs/nvidia/hopper/ref-docs/gluon/linear_attention.md` | Chunk Linear Attention / Recurrent optimization topic |
| `softmax_reduce.md` | Softmax / Reduction / Element-wise optimization topic |
| `fused_attention.md` | Fused Attention optimization topic |

### Cross-Reference (hopper-docs/ guide)

| Document | Referenced Content |
|----------|--------------------|
| `docs/nvidia/hopper/api_mapping.md` | Hopper Gluon API reference |
| `docs/nvidia/hopper/layouts.md` | Hopper Layout types and TTGIR mapping |
| `docs/nvidia/hopper/pipeline.md` | async_copy pipeline implementation patterns |
| `docs/nvidia/hopper/memory_access.md` | Hopper memory access patterns |
| `docs/nvidia/hopper/matrix_multiply.md` | wgmma matrix multiplication patterns |
| `docs/nvidia/hopper/common_pitfalls.md` | Hopper common pitfalls and resolutions |
