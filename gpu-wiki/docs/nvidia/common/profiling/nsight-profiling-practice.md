...
- [Profiling Tools by Architecture](profiling-tools-by-arch.md) — Profiling tool support for different GPU architectures
- [Occupancy Tuning](../occupancy-tuning-by-arch.md) — Occupancy calculation and tuning strategies by architecture
- [NVIDIA Compute Capabilities](../nvidia-compute-capabilities.md) — SM resource limits (registers/shared memory/warp count)
- [PTX Programming Model](../ptx/ptx-programming-model.md) — Understanding PTX instructionsʰ to assist with SASS-level analysis
