...
Comprehensive core knowledge from the Zhihu community on optimizing common GPU operators (softmax, LayerNorm, reduce, etc.). Supplements CUDA-level operator optimization details not covered in [Online Softmax & Flash Attention Hands-on](hands-on/online-softmax-flash-attention.md) and [Fused Kernel Patterns Hands-on](hands-on/fused-kernel-patterns.md).
...
- [Online Softmax and Flash Attention Hands-On](hands-on/online-softmax-flash-attention.md) -- Triton implementation of the online softmax algorithm and Flash Attention
- [Fused Kernel Patterns Hands-On](hands-on/fused-kernel-patterns.md) -- A systematic methodology for kernel fusion
