...
[cdna3.md](../../cdna3/converter/cdna3.md) and apply the CDNA4 deltas below. Source: `reference-projects/triton` (gluon @
...
3. Everything in [cdna3.md](../../cdna3/converter/cdna3.md) pitfalls also applies (layouts from TTGIR `hip:gfx950`, no `make_block_ptr`, `in_thread_transpose` handling).

## Common failures (symptom → cause → fix)
Inherits all CDNA3 failures — see [cdna3.md](../../cdna3/converter/cdna3.md). CDNA4 deltas:
| Symptom | Cause → Fix |
|---------|-------------|
