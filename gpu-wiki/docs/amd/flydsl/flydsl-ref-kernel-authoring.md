...
> **API**: This guide documents the FlyDSL/FlyDSL2 API from early and later development phases (pre-JIT).

## Quick Reference

| Concept | API | Description |
|---|---|---|
| **JIT host func** | `flydsl.jit` | Emit host-side launcher with JIT compilation |
| **GPU kernel** | `flydsl.gpu.kernel` | Define GPU kernel function |
| **Launch** | `flydsl.gpu.launch` | Configure and emit GPU launch |
| **Thread ID** | `flydsl.gpu.thread_id` | Get thread index in workgroup |
| **Block ID** | `flydsl.gpu.block_id` | Get block/workgroup index |
| **Block dim** | `flydsl.gpu.block_dim` | Get block dimension size |
| **Compile-time** | `flydsl.compile_time` | Compile-time constant parameter |
| **Tensor arg** | `flydsl.tensor_arg` | GPU tensor argument (via DLPack) |
| **Stream arg** | `flydsl.stream_arg` | CUDA/HIP stream argument |
| **Barrier** | `flydsl.gpu.barrier` | Workgroup synchronization |
| **Constants** | `flydsl.mlir.arith.constant` | Create MLIR constant value |
| **Range loop** | `flydsl.range` | Compile-time unrolled loop |
| **Buffer load** | `flydsl.amd.buffer_load` | AMD buffer load intrinsic |

---

## 1. Basic Kernel Pattern

### 1.1 `flydsl.jit` + `flydsl.gpu.kernel`

```python
@flydsl.jit
def add_kernel(a: flydsl.tensor_arg(shape=(256,), dtype="float32"),
               b: flydsl.tensor_arg(shape=(256,), dtype="float32"),
               c: flydsl.tensor_arg(shape=(256,), dtype="float32"),
               stream: flydsl.stream_arg = None):
    tid = flydsl.gpu.thread_id(0)
    a_val = a[tid]
    b_val = b[tid]
    c[tid] = a_val + b ain_val
```

### 1.2 How It Works

1. `flydsl.jit` wraps the function as a `FlyJitFunction`
2. `flydsl.gpu.kernel` wraps the function as a `FlyGPUKernel`
3. On first call, the `FlyJitFunction` triggers:
   - AST rewriting (Python loops/ifs → MLIR scf ops)
   - MLIR module creation with `gpu.module`
   - Tracing the jit function body to generate MLIR ops
   - Calling `flydsl.gpu.launch` emits a `gpu.launch_func` in the host module
   - `flydsl.gpu.kernel` emits `gpu.func`
   - `pass_pipeline_jit` runs the full pass pipeline
   - `ExecutionEngine` wraps the resulting ExecutionEngine
...
- [FlyDSL inline_asm Writing Guide (AMD GFX942 / gfx1250)](flydsl-inline-asm-patterns.md)
- [FlyDSL Layout Algebra Reference](flydsl-layout-algebra.md)
- [FlyDSL Pre-built Kernel Library Reference](flydsl-prebuilt-kernels.md)
- [FlyDSL Programming Guide](flydsl-programming-guide.md)
- [[FlyDSL Original Documentation] Python API Reference](flydsl-ref-api.md)
