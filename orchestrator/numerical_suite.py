...
from .infrastructure_retry import check_review_service, retry_review
...
def resolve_suite(campaign, workspace, private):
    ...
    def author_once():
        with tempfile.TemporaryDirectory(prefix="atrex-numerical-contract-") as temporary:
            ...
            result = run_session(root, PROMPT.read_text(), timeout=getattr(campaign, "numerical_review_timeout", 600),
    ...
    retry_review(workspace, f"numerical-suite:{digest.hexdigest()}:{campaign.agent_cli}:{getattr(campaign, 'numerical_review_timeout', 600)}", author_once)
    return cached
