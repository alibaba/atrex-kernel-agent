...
class HostSessionTranscripts:
        ...
        *,
        ...
        if backend == "codex" and "resume" in command:
            self.session_id = next(
                (arg for arg in command if re.fullmatch(r"[a-fA-F0-9-]{32,64}", arg)), ""
            )
    ...
    def _discovered_paths(self, *, force: bool) -> tuple[Path, ...]:
        ...
        if force or now >= self._next_discovery:
            ...
            self._metadata = {
                path: value for path, value in self._metadata.items() if path in retained
            }
            self._next_discovery = time.monotonic() + DISCOVERY_INTERVAL_SECONDS
    ...
    def _paths(self):
        ...
        for pattern in self.patterns:
            for path in self.root.glob(pattern):
                ...
                components = [
                    path,
                    *(parent for parent in path.parents if parent.is_relative_to(self.root)),
                ]
