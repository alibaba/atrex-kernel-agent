...
from orchestrator.test_unsandboxed_usage import (
    CHILD_ID,
    ROOT_ID,
    codex_stream,
    count,
...
class SessionDiscoveryTests(unittest.TestCase):
    ...
    def discovery(self, **kwargs):
        return HostSessionTranscripts(
            "codex", self.environment, ["codex", "resume", ROOT_ID], **kwargs
        )
    ...
    def test_stable_headers_and_negative_results_are_cached_between_scans(self):
        ...
        with (
            ...
            patch.object(
                session_native, "_regular_bytes", wraps=session_native._regular_bytes
            ) as reads,
        ):
        ...
        for replace_file in (False, True):
            with self.subTest(replace_file=replace_file):
                ...
                capture = SessionCapture(
                    self.root / "captures",
                    backend="codex",
                    command=["codex", "resume", ROOT_ID, "prompt"],
                    provider_home=None,
