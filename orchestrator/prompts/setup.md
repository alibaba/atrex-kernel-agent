...
3. **Stage 1 — Baseline.** {{BASELINE_DRIVER}}: implement `kernel.py` + `test_kernel.py`,
   ...
   If a subagent is used, include the mandatory sandbox block above verbatim in its task. The baseline worker must run `test_kernel.py
