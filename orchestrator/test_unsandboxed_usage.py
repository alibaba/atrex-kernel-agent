...
from orchestrator.session_capture import (
    SessionCapture,
    ...
    return {
        ...
        "payload": {
            ...
            "info": {
                ...
                "last_token_usage": last or counters,
            },
        },
    }
...
class UnsandboxedUsageTests(unittest.TestCase):
    ...
    def test_captures_configured_claude_home_and_children_without_supervisor(self):
        ...
        capture = start_session_capture(
            ["claude", "--session-id", "main", "prompt"], workspace, self.environment
        )
        self.assertIsNone(capture.home)
        self.assertEqual(self.environment["CLAUDE_CONFIG_DIR"], str(configured))
        self.write(configured / "projects/p/main.jsonl", lines(assistant()))
        self.write(
            configured / "projects/p/main/subagents/child.jsonl",
            lines(assistant("child-msg", agent="child")),
        )
        ...
        self.assertNotIn("unrelated", (capture.root / "conversation.jsonl").read_text())
    ...
    def test_none_missing_native_usage_remains_partial(self):
        report = self.finish(
            self.capture(), lines(assistant(), {"type": "result", "usage": usage()})
        )
    ...
    def test_qoder_and_pi_capture_only_the_requested_session(self):
        for backend, relative in (
            ("qodercli", ".qoder/tasks/main.jsonl"),
            ("pi", ".pi/agent/sessions/p/date_main.jsonl"),
        ):
            with self.subTest(backend=backend):
                ...
                report = self.finish(
                    capture, lines({"type": "result", "usage": usage(), "credits": 1.25})
                )
                self.assertEqual(report["response_count"], 1)
                if backend == "qodercli":
    ...
    def test_complete_codex_family_capture_wins_over_root_only_ledger(self):
        ...
        _, total, _, _ = normalize_stream(
            "codex", codex_stream(), session_id=ROOT_ID, codex_observer=observer
        )
        ...
        _, total, _, _ = normalize_stream(
            "codex", codex_stream(110, 11, 22), session_id=ROOT_ID, codex_observer=observer
        )
        self.assertEqual(total.measurement, "exact")
    ...
    def test_partial_codex_family_is_not_replaced_by_root_only_ledger(self):
        ...
        self.write(
            ...
            lines(metadata(CHILD_ID, ROOT_ID), count(20, 2, 4)) + '{"incomplete":',
        )
        self.finish(capture, codex_stream())
        observer = CodexSessionLedgerObserver(self.home / ".codex")
        _, total, _, errors = normalize_stream(
            "codex", codex_stream(), session_id=ROOT_ID, codex_observer=observer
        )
    ...
    def test_failed_cursor_advance_cannot_replay_an_accounted_invocation(self):
        ...
        observer = CodexSessionLedgerObserver(self.home / ".codex")
        with patch.object(
            observer, "observe_reconciled", side_effect=ValueError("temporary ledger failure")
        ):
            _, total, _, _ = normalize_stream(
                "codex", codex_stream(), session_id=ROOT_ID, codex_observer=observer
            )
    ...
    def test_codex_partial_capture_does_not_block_ledger_in_either_entrypoint(self):
        ...
        self.write(
            codex_home / f"sessions/rollout-{ROOT_ID}.jsonl", lines(metadata(ROOT_ID), count())
        )
        stream = codex_stream()
        ...
        events, total, capabilities, errors = normalize_stream(
            "codex", stream, session_id=ROOT_ID, codex_observer=observer
        )
        self.assertEqual(total.measurement, "exact")
        self.assertEqual(
            (total.input_tokens, total.cache_read_tokens, total.total_tokens), (80, 20, 110)
        )
    ...
    def test_incomplete_capture_is_preserved_independently_of_error_wording(self):
        ...
        for label in ("stdout_capture_write", "codex_session_identity", "storage_write_failure"):
            with self.subTest(label=label):
                ...
                observed = captured_observation(
                    stream, (), AgentRuntimeCapabilities(True, True, False)
                )
                ...
                for entrypoint, (
                    ...
                    capabilities,
                    errors,
                ) in self.codex_entrypoint_observations(stream, home).items():
    ...
    def test_warning_text_cannot_block_healthy_stream_only_ledger_fallback(self):
        ...
        def report_with_note(*args, **kwargs):
            ...
            report["warnings"].append(
                "provider note: capture pipe delivered terminal counters only"
            )
            return report
        ...
        for entrypoint, (_, total, capabilities, errors) in self.codex_entrypoint_observations(
            stream, home
        ).items():
            with self.subTest(entrypoint=entrypoint):
                self.assertEqual(total.measurement, "exact")
    ...
    def test_real_codex_child_capture_without_supervisor_or_bwrap(self):
        workspace = self.root / "workspace"
        workspace.mkdir()
        ...
        with (
            patch.dict(
                os.environ,
                {
                    ...
                    "ATREX_SESSION_CAPTURE_DIR": str(capture_root),
                },
            ),
        ...
        self.assertEqual(result.session_id, ROOT_ID)
        reports = list(capture_root.glob("*/token-usage.json"))
        ...
        self.assertEqual(report["response_count"], 2)
        self.assertEqual(
            len(list(reports[0].parent.glob("provider/native/.codex/sessions/*.jsonl"))), 2
        )
