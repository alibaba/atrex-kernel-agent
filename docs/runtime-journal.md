...
After report/evidence validation and before creating the candidate commit or finalizing the Journal, the Supervisor selects an evaluator-specific acceptance gate. For Atrex-Bench it looks for a successful full or correctness-only Evaluate with six correctness cases and matching frozen Kernel, complete input/Shape contract, evaluator, tolerances, target and environment. Ordinary Atrex-Bench Evaluate supplies six cases by default. Only timing iterations, performance repetitions and full versus correctness-only mode may differ for this correctness-only reuse; custom inputs, explicit Shape subsets, fewer seeds and old records without the resolved policy cannot substitute. If none qualifies, the Supervisor requests `--kind run --mode correctness_only --multi-seed 5 --no-sync`.

SOL-ExecBench instead runs or exactly reuses `--kind run --no-sync`. This invokes the official evaluator over every entry in `workload.jsonl`, using its per-workload tolerances. The request deliberately contains neither `--mode correctness_only` nor `--multi-seed`: those controls are Atrex-Bench typed options and would prevent the SOL Dev fallback. A qualifying SOL record must bind the exact candidate source, `reference.py`, `definition.json`, `workload.jsonl`, `solution.json`, the trusted `test_kernel.py`, and optional `config.json`; it must report positive, complete workload coverage and an all-pass result. The private Journal retains the successful Record under the compatibility key `acceptance_checks.multi_seed_correctness`; the Agent need not add an extra check to its Experiment. Agent duplicate-request errors remain unchanged.

SOL Agent evaluation and acceptance share the `sol_execbench_coverage_v1` result contract in task identity. Upgrading a store with earlier unversioned SOL records automatically measures the first matching task again under the configured repetition policy; it does not repeatedly reuse a response whose coverage was discarded. Old Records remain readable, and new matching Records are reused normally. See [measurement upgrade behavior](measurement-records.md#optional-repeated-measurement).

SOL measurements use the repository-trusted `reference/test_kernel.py` and the V0-committed
`config.json`, replacing mutable copies before hashing and execution. Acceptance verifies the
harness digest, not merely its presence in the Record. Configuration changes in an Agent draft,
worktree or index cannot change this policy; an absent V0 config stays absent. Invalid or missing
Git provenance stops dispatch rather than trusting the mutable copy.
