...
Setup, Fast/Full, plan/profile artifacts, Wiki attribution, Phase Markers and gate policy remain. In managed optimization Episodes, the Supervisor validates exact measured source, creates the candidate commit, and publishes private compatibility state; the Agent supplies no Commit ID. Fast-mode experiment/evaluation minimums apply to `candidate_ready` and `pivot`; `blocked` is exempt at both report submission and completion checks, but still requires a nonempty blocker and all other report/lifecycle checks. Permitted-operation rules remain unchanged. The new interface's explicit behavior changes are evidence-linked Direction lifecycle checks and permission to submit an empty `pivot` in Full mode, or an empty `blocked` in either mode, when no Direction needs closing; an empty Fast-mode pivot still fails its minimums.
...
Local verification covered 29 Journal regressions, 102 existing measurement/Gateway regressions and 4 existing Wiki-attribution checks. In particular, the compatibility projection passes the existing Long Horizon completion check and memory compiler, Fast-mode minimums remain enforced for non-blocked outcomes, and candidate reports reject correctness-only or Shape-subset evidence. Private-request canaries are absent from the internal evidence view, Agent query responses and compatibility report; deriving eligibility preserves custom-input and Shape-subset exclusions. No new test modules are included in the source tree.

An additional 24 projection-trust regressions cover forged workspace flags, absent/corrupt/unfinalized private records, identity and terminal-field mismatches, empty or partial Experiment omissions, unknown/substituted IDs, malformed rows and IDs, duplicate-ID count inflation, valid complete ID sets, valid empty outcomes, strict legacy validation, unchanged Fast minimums and recovery before Episode registration. These fixtures also remain outside the repository.

Sixteen focused ancestry/Fast-policy regressions verify the three relationship kinds, parent visibility and cycle checks, correction/combination constraints, repairable rejection of unsupported kinds, and same-ID restart. They also check the blocked-count exemption at both report submission and completion, retained blocker/lifecycle validation, and non-blocked minimum enforcement.
