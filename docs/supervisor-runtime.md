...
This execution boundary does **not change the optimization workflow**. Setup/Framework Baseline, Fast/Full Episodes, phase markers, plans, profiles, Agent Git commits, Journal and acceptance remain unchanged. [Measurement records and recovery](measurement-records.md) add private Kernel/Measurement IDs, cross-Episode deduplication, infrastructure retry and optional repeated measurement. Runtime Journal/Report and Supervisor-owned promotion remain separate changes.
