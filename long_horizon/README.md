...
After at least 50 completed optimization episodes, more than 3 consecutive episodes without
promotion automatically select `goal` mode for the next episode. Baseline versions do not count.
Goal mode takes precedence over fast mode and the optional stall stop; an earlier `--max-stall`
stop can still end a campaign before it reaches episode 50. Explicit iteration, episode, and token
budgets continue to limit admission of new episodes.

Goal episodes pursue an evolving roadmap across multiple optimization categories and may redesign
or substantially refactor `kernel.py`. Initial improvements become recoverable checkpoints while
the agent explores remaining worthwhile opportunities. Plans and both reviewer backends receive the
same goal scope; reviewer enablement follows the full-episode settings. There is no single-episode
wall-clock limit or fixed trial count, and resumable backends receive at least 20 incomplete-handoff
...
requirements, and independent ABBA correctness/performance promotion checks apply in goal mode.
